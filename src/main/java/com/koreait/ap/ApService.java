...
public class ApService {
    ...
    public List<ApartmentInfoEntity> selApartmentInfoList(SearchDto dto) {
    ...
    public  int insApartmentInfo(ApartmentInfoEntity[] list, SearchDto dto) {
        List<ApartmentInfoEntity> slist =  mapper.selApartmentInfoList(dto);
        if(slist.size() == 0) {
            for (ApartmentInfoEntity item : list) {
                LocationCodeEntity codeEntity = mapper.selLocation(dto.getExcd());
                item.setLocationcode(codeEntity.getIcd());
                item.setExcd(codeEntity.getExcd());
                item.setDealamout(item.getDealamout().replaceAll(",", ""));
